...
Cmu800Rhythm::Cmu800Rhythm(const int8_t* sample_data,
...
void Cmu800Rhythm::set_sample(const int8_t* sample_data,
	uint32_t sample_count)
{
...
void Cmu800Rhythm::set_sample_rate(uint32_t sample_rate)
{
	this->sample_rate = sample_rate != 0 ? sample_rate : 48000u;
	step_q32 = (static_cast<uint64_t>(48000u) << 32) / this->sample_rate;
}
...
int32_t Cmu800Rhythm::get_data(int32_t volume_q15)
{
	const uint64_t position = position_q32 >> 32;
	...
	const uint32_t fraction =
		static_cast<uint32_t>((position_q32 >> 16) & 0xffffu);
	const int32_t weighted =
		static_cast<int32_t>(sample_data[index]) *
			static_cast<int32_t>(65536u - fraction) +
		static_cast<int32_t>(sample_data[next_index]) *
